...
working_dir = "/media/data04/gaye/workspace/rnaseq/de/ballgown/ref_only" 
setwd(working_dir)
...
gene_expression[1:3, c("FPKM.SRR12424243","FPKM.SRR12424244","FPKM.SRR12424245","FPKM.SRR12424257")]
...
#View expression values for the transcripts of a DE top gene symbol e.g. 'FTX'
#First determine the rows in the data.frame that match 'FTX', aka. ENSG00000230590, then display only those rows of the data.frame
i = row.names(gene_expression) == "ENSG00000230590"
gene_expression[i,]

#What if we want to view values for a list of DE top 4 genes of interest all at once? 
genes_of_interest = c("ENSG00000230590","ENSG00000118523","ENSG00000101346","ENSG00000261557")
...
min(gene_expression[,"FPKM.SRR12424243"])
max(gene_expression[,"FPKM.SRR12424243"])
...
data_columns=c(1:6)
short_names=c("SRR12424243","SRR12424244","SRR12424245","SRR12424255","SRR12424256","SRR12424257")
...
x = gene_expression[,"FPKM.SRR12424243"]
y = gene_expression[,"FPKM.SRR12424244"]
plot(x=log2(x+min_nonzero), y=log2(y+min_nonzero), pch=16, col="blue", cex=0.25, xlab="FPKM (NORMAL, SRR12424243)", ylab="FPKM (NORMAL, SRR12424244)", main="Comparison of expression values for a pair of replicates")
...
smoothScatter(x=log2(x+min_nonzero), y=log2(y+min_nonzero), xlab="FPKM (NORMAL, SRR12424243)", ylab="FPKM (NORMAL, SRR12424244)", main="Comparison of expression values for a pair of replicates", colramp=colors, nbin=200)
...
plotCor("FPKM.SRR12424243", "FPKM.SRR12424255", "SRR12424243 vs SRR12424255", "tomato2")
plotCor("FPKM.SRR12424244", "FPKM.SRR12424256", "SRR12424244 vs SRR12424256", "royalblue2")
...
par(mfrow=c(1,2))
plotCor2("FPKM.SRR12424243", "FPKM.SRR12424255", "SRR12424243 vs SRR12424255", "tomato2")
plotCor2("FPKM.SRR12424244", "FPKM.SRR12424256", "SRR12424244 vs SRR12424256", "royalblue2")
...
par(mfrow=c(1,1))
plot(mds$points, type="n", xlab="", ylab="", main="MDS distance plot (all non-zero genes)", xlim=c(-0.27,0.27), ylim=c(-0.1,0.1))
...
hist(results_genes[sig,"de"], breaks=50, col="seagreen", xlab="log2(Fold change) NORMAL vs INFECTED", main="Distribution of differential expression values")
...
gene_expression[,"NORMAL"]=apply(gene_expression[,c(1:3)], 1, mean)
gene_expression[,"INFECTED"]=apply(gene_expression[,c(4:6)], 1, mean)

x=log2(gene_expression[,"NORMAL"]+min_nonzero)
y=log2(gene_expression[,"INFECTED"]+min_nonzero)
plot(x=x, y=y, pch=16, cex=0.25, xlab="NORMAL FPKM (log2)", ylab="INFECTED FPKM (log2)", main="NORMAL vs INFECTED FPKMs")
abline(a=0, b=1)
xsig=x[sig]
ysig=y[sig]
